...
async def get_hedgefund_agents():
    try:
        from tradingagents.hedgefund.utils.analysts import get_agents_list
...
async def start_hedgefund_analysis(req: HedgeFundRequest):
    ...
    def run_job():
        try:
            from tradingagents.hedgefund.main import create_workflow, parse_hedge_fund_response
            from langchain_core.messages import HumanMessage
            from datetime import datetime
            from dateutil.relativedelta import relativedelta

            start_dt = req.start_date or (datetime.now() - relativedelta(months=3)).strftime("%Y-%m-%d")
            end_dt = req.end_date or datetime.now().strftime("%Y-%m-%d")
            ...
            workflow = create_workflow(req.selected_analysts)
            ...
            _hedgefund_jobs[job_id]["status"] = "failed"
            _hedgefund_jobs[job_id]["error"] = str(e)
            _hedgefund_queues[job_id].append({"event": "error", "data": str(e)})

    threading.Thread(target=run_job, daemon=True).start()
    return {"job_id": job_id}


@app.get("/api/hedgefund/stream/{job_id}")
async def stream_hedgefund_analysis(job_id: str):
    if job_id not in _hedgefund_jobs:
        raise HTTPException(status_code=404, detail="Job not found")
